...
- `/api/status` reports live XS heap usage under `instruments` when the host
  ...
  branch now also builds with a 1.25MB XS arena — the previous 432KB arena was
  57% full at boot and exhausted itself after ~3.5 hours of match-day load
  (serial signature: "Chunk allocation ... failed" storms, then reset). The
  mod still runs unchanged on older or stock hosts; both features degrade
  gracefully. The abort hook also falls back to persisting the bare status
  when heap exhaustion makes the full reason string unallocatable.
...
- host 带 instrumentation 模块（仪表化构建）时，`/api/status` 在 `instruments`
  字段实时报告 XS 堆用量。matchday host 分支同时把 XS 堆加大到 1.25MB——旧的
  432KB 开机即占 57%，比赛负载约 3.5 小时后耗尽（串口签名：连串 "Chunk
  allocation ... failed" 后复位）。mod 在旧 host / 官方 host 上照常运行，两项
  能力都优雅降级。abort 钩子在堆耗尽导致完整原因串无法分配时，退而保存裸状态
  字符串。
