...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;letter-spacing:0}body{margin:0;background:var(--bg);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    .segment{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border:1px solid var(--line);border-radius:6px;overflow:hidden;background:#fff}.segment label{min-width:0}.segment input{position:absolute;width:1px;height:1px;opacity:0}.segment span{display:flex;align-items:center;justify-content:center;min-height:44px;padding:6px 4px;font-size:13px;text-align:center;border-right:1px solid var(--line);overflow-wrap:anywhere}.segment label:last-child span{border-right:0}.segment input:checked+span{background:#17202a;color:#fff}
    button.primary{margin-top:16px;background:var(--red);border-color:var(--red);color:#fff;text-align:center}.message{min-height:22px;margin-top:10px;font-size:13px;color:var(--muted)}.message.ok{color:var(--green)}.message.error{color:#b42318}.empty,.current{font-size:13px;line-height:1.6;color:var(--muted)}
    @media(max-width:420px){.match{grid-template-columns:1fr}.match time{justify-self:start}}
  </style>
</head>
<body><main>
  <header><h1>Stack-chan 赛前设置</h1><div class="status" id="health">设备在线</div></header>
  <section><h2>未来比赛</h2><div class="matches" id="matches"><div class="empty">等待 watcher 推送赛程</div></div></section>
  <section class="form" id="form"><div class="versus" id="versus"></div><span class="field">支持球队</span><div class="segment" id="favorite"></div><span class="field">赛前持仓</span><div class="segment" id="position"></div><button class="primary" id="apply">开始看球</button><div class="message" id="message"></div></section>
  <section><h2>当前监控</h2><div class="current" id="current">读取中</div></section>
</main>
<script>
const state={selected:null,lastResult:''};const $=id=>document.getElementById(id);const localTime=value=>new Intl.DateTimeFormat('zh-CN',{weekday:'short',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(value));
function message(text,kind=''){const el=$('message');el.textContent=text;el.className='message '+kind}
function choices(rootId,name,teams,empty){const root=$(rootId);root.textContent='';[...teams,{name:'',localized:empty}].forEach((team,index)=>{const label=document.createElement('label');label.innerHTML='<input type="radio" name="'+name+'" value="'+team.name+'" '+(index===teams.length?'checked':'')+'><span>'+team.localized+'</span>';root.appendChild(label)})}
function choose(match){state.selected=match;document.querySelectorAll('.match').forEach(el=>el.classList.toggle('selected',el.dataset.id===match.event_id));const teams=[match.home,match.away];$('versus').textContent=match.label;choices('favorite','favorite_team',teams,'中立');choices('position','position_team',teams,'没买');$('form').classList.add('show');message('')}
function render(data){const root=$('matches');root.textContent='';if(!data.options.length)root.innerHTML='<div class="empty">watcher 暂无开放盘口</div>';data.options.forEach(match=>{const button=document.createElement('button');button.className='match';button.dataset.id=match.event_id;button.innerHTML='<strong>'+match.label+'</strong><time>'+localTime(match.starts_at)+'</time>';button.onclick=()=>choose(match);root.appendChild(button)});const current=data.current||{};$('current').textContent=current.label?(current.label+' · 支持 '+(current.favorite_team||'中立')+' · 持仓 '+(current.position_team||'无')):'尚未配置';if(data.lastResult&&data.lastResult.request_id!==state.lastResult){state.lastResult=data.lastResult.request_id;if(data.lastResult.ok)message(data.lastResult.label+' 已开始监控','ok');else message(data.lastResult.error||'watcher 设置失败','error')}}
async function refresh(){try{const response=await fetch('/api/match-setup');render(await response.json());$('health').textContent='设备在线'}catch(error){$('health').textContent='连接失败'}}
$('apply').onclick=async()=>{if(!state.selected)return;const favorite=document.querySelector('input[name="favorite_team"]:checked')?.value||'';const position=document.querySelector('input[name="position_team"]:checked')?.value||'';const payload={request_id:String(Date.now()),event_ticker:state.selected.kalshi_event_ticker,espn_event_id:state.selected.event_id,favorite_team:favorite,position_team:position};message('已提交，等待 watcher');try{const response=await fetch('/api/match-setup/apply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json();if(!response.ok)throw new Error(result.error||'提交失败')}catch(error){message(error.message,'error')}};
...
export function startHttp(robot) {
  ...
  server.get('/api/match-setup/pending', () => json({ pending: state.matchSetup.pending }))
